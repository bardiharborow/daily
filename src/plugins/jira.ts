...
function dateToJql(date: Date): string {
...
export class Jira implements Plugin {
  ...
  async fetchUpdatesByCategory(since: Date): Promise<UpdatesByCategory> {
    ...
    for (const issue of issues) {
      ...
      const line = `${issue.key}: ${issue.fields.summary.trim()} – \`${issue.fields.status.name}\` – ${emoji}`;
  ...
  private async fetchRelevantIssues(since: Date): Promise<JiraIssue[]> {
    const jqlDate = dateToJql(since);
